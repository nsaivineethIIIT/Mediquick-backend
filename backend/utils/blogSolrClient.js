...
class BlogSolrClient {
  ...
  async init() {
    try {
      ...
      if (!exists) {
        this.ready = false;
        console.warn('⚠️ Blog Solr core is not ready yet');
        return;
      }
      this.ready = true;
      console.log('✅ Blog Solr core is ready');
    } catch (err) {
      console.warn('⚠️ Blog Solr initialization issue:', err.message);
      this.ready = false;
    }
  }
      ...
      const req = client.request({
        ...
        res.on('end', () => {
          try {
            ...
            if (!ok) {
              console.warn('⚠️ Blog Solr index failed:', response?.error?.msg || data);
            }
            resolve(ok);
          } catch (e) {
            resolve(false);
  ...
  async search(query, filters = {}, start = 0, rows = 10) {
    ...
    return new Promise((resolve) => {
      const fq = ['entityType_s:blog'];
      if (filters.theme) {
        fq.push(`theme_s:\"${filters.theme}\"`);
      }
      ...
      const qf = encodeURIComponent('search_text_t title_s^5 theme_s^3 authorName_s^2 content_t');
        ...
        res.on('end', () => {
          try {
            ...
            const docs = rawDocs.map((doc) => ({
              ...
              authorName: doc.authorName_s,
              authorType: doc.authorType_s,
              image: doc.image_s,
              createdAt: doc.createdAt_dt
            }));
...
module.exports = new BlogSolrClient();
