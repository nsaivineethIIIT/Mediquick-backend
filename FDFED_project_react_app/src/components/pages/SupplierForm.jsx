...
const SupplierForm = () => {
  ...
  const signupForm = useForm({
    ...
    mode: 'onChange',
    defaultValues: { supplierID: '', name: '', signupEmail: '', mobile: '', address: '', signupPassword: '', signupSecurityCode: '', profilePhoto: null, document: null }
  });
  ...
  const handleLogin = async (data) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setTimeout(() => { window.location.href = result.redirect || '/supplier/dashboard'; }, 1000);
      } else {
      ...
      <header className="fixed top-0 w-full z-50 bg-slate-50/80 backdrop-blur-xl shadow-sm">
        <div className="flex justify-between items-center px-6 md:px-10 py-4 max-w-[1440px] mx-auto w-full">
          ...
          <div className="hidden md:flex gap-8 items-center">
            <Link to="/supplier/dashboard" className="font-manrope font-bold text-sm md:text-base tracking-tight text-slate-500 hover:text-blue-600 transition-colors">Dashboard</Link>
      ...
      <main className="pf-main">
        ...
        <div className="pf-layout">
          <aside className="pf-content-side">
            ...
            <p>Supply quality medicines, reach more patients, and grow your business with our comprehensive platform.</p>
          </aside>
          ...
          <section className="pf-form-side">
            <div className="pf-card">
              ...
              <h2 className="pf-form-title">{isLogin ? 'Supplier Login' : 'Supplier Registration'}</h2>

              {message.text && <div className={`pf-message ${message.type}`}>{message.text}</div>}
                ...
                <form onSubmit={handleSubmit(handleLogin)} className="pf-form-stack">
                  <div className="pf-field">
                    ...
                    <div className="pf-input-wrap">
                      ...
                      <input type="email" className={`pf-input ${errors.email ? 'is-error' : ''}`} placeholder="supplier@example.com" {...register('email')} autoComplete="email" />
                    </div>
                  ...
                  <button type="submit" className="pf-submit-btn" disabled={isLoading}>{isLoading ? 'Logging in...' : 'Login to Portal'}</button>
                  <p className="pf-terms-text">New here? <span className="pf-link" onClick={toggleForm}>Create your supplier account</span></p>
                </form>
              ) : (
                <form onSubmit={handleSubmit(handleSignup)} className="pf-form-stack">
                  <div className="pf-grid-2">
                    <div className="pf-field">
                      <label className="pf-label">Supplier ID</label>
                      <div className="pf-input-wrap">
                        <span className="material-symbols-outlined pf-input-icon">badge</span>
                        <input type="text" className={`pf-input ${errors.supplierID ? 'is-error' : ''}`} placeholder="e.g., SUP12345" {...register('supplierID')} />
                      </div>
                      {errors.supplierID && <span className="pf-error-message">{errors.supplierID.message}</span>}
                    </div>
                    <div className="pf-field">
                  ...
                  <div className="pf-grid-2">
                    <div className="pf-field">
                      ...
                      <div className="pf-input-wrap">
                        <span className="material-symbols-outlined pf-input-icon">mail</span>
                        <input type="email" className={`pf-input ${errors.signupEmail ? 'is-error' : ''}`} placeholder="supplier@example.com" {...register('signupEmail')} />
                      </div>
                  ...
                  <button type="submit" className="pf-submit-btn" disabled={isLoading}>{isLoading ? 'Creating account...' : 'Create Supplier Account'}</button>
